	...
	6、kernel分析
		...
		Head.S文件分析
			...
			二、C语言阶段
				...
				四、文件系统
					为什么需要跟文件系统
						1、 init进程的应用程序在跟文件系统上
						2、 跟文件系统提供了根目录/
						3、 内核启动后的应用层配置（/etc目录）在跟文件系统上。几乎可以认为：发行版 = linux内核 + 跟文件系统
						4、 shell命令程序在跟文件系统
						...

						总结：一套linux体系，只有内核本省是不能工作的，必须要rootfs（上的etc目录下的配置文件、/bin /sbin等目录下的shell命令，还有/lib目录下的库文件等...）相配合才能工作。

					跟文件系统的实质是什么
						1、 跟文件系统是特殊用途的文件系统。
						2、 跟文件系统也必须属于某种文件系统格式
						3、 究竟文件系统是用来干嘛的

						总结：首先，存储设备（块设备、像硬盘、flash等）是分块（扇区）的，物理上底层去访问存储设备时按照块号（扇区号）来访问。这就很麻烦。
							其次，文件系统是一些代码，是一套软件，这套软件的功能就是对存储设备的扇区进行管理，将这些扇区的访问编程了对目录和文件名的访问。我们在上层按照特定的目录和文件名去访问一个文件时，文件系统会将这个目录+文件名转换成对扇区号的访问。
							最后，不同的文件系统的差异就在于对这些扇区的管理策略和方法不同，譬如坏块管理、碎片管理。

					跟文件系统的形式
						镜像文件形式
							1、 使用专用工具软件制作的可烧录的镜像文件
							2、 镜像中包含了跟文件系统中的所有文件
							3、 烧录此镜像类似于对相应分区格式化。
							4、 镜像文件系统具有一定的格式，格式是内化的，跟文件名后缀是无关的。

						文件夹形式
							1、 跟文件系统其实就是一个包含特定内容的文件夹而已。
							2、 跟文件系统可由任何一个空文件夹添加必要文件构成而成。
							3、 跟文件系统的雏形就是在开发主机中构造的文件夹形式的

						总结：
							1、 镜像文件形式的跟文件系统主要目的是用来烧录到块设备上，设备上的内核启动后去挂在它。镜像文件形式的跟文件系统是由文件夹形式的跟文件系统使用专用的镜像制作工具制作而成的。
							2、 最初在开发主机中随便mkdir创建一个空文件夹，然后向其中添加一些必要的文件（包括etc目录下的运行时配置文件、/bin等目录下的可执行程序、/lib目录下的库文件等...）后就形成了一个文件夹形式的rootfs。然后这个文件夹形式的rootfs可以被kernel通过nfs方式来远程挂在使用，但是不能用来烧录块设备。我们为了将这个rootfs烧录到块设备中于是用一些专用的软件工具将其制作成可供烧录的一定格式的跟文件系统镜像。
							3、 文件夹形式的rootfs是没有格式的，制作成镜像后就有了一定的rootfs格式了，格式是由我们的镜像制作过程工具决定的。每一种格式的镜像工具的用法都不同。

					1、自己制作ext3格式的跟文件系统
						(1) mke2fs介绍
							1）mke2fs是一个应用程序，ubuntu默认安装了，作用：用来制作ext2、ext3、ext4等格式的更文件系统的。
							2）一般用来制作各种不同格式的rootfs的应用程序的名字都很相似，类似于 mkfs.xx（譬如：mkfs.ext2、 mkfs.jffs2，mkfs.xxx --> mke2fs的符号链接）
						...
						(2) 动手制作ext3格式的跟文件系统
							...
							1）dd if=/dev/zero of=rootfs.ext2 bs=1024 count=2048		// 创建一个 2M 大小的空文件
