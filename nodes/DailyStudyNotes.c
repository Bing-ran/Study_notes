栈指针不是四字节对齐则无法调用c程序。（32位处理器4字节对齐）
...
SLSA驱动框架分析
	1、 基础名词
		1）PCM（Pulse Code Modulation）即脉冲编码调制
				在PCM过程中，将输入的模拟信号进行采样、量化和编码，PCM信号的两个重要指标是采样频率和量化精度。
	...
	2、 ALSA驱动框架分析
		1）ALSA Lib
				ALSA还包含在用户空间的alsa-lib函数库，具有更加友好的编程接口，开发者可以通过这些高级API编写应用程序，不必直接操作设备文件。

		2）ALSA Core 
				ALSA核心层，抽象了一套数据结构与接口，向应用层提供了统一的编程接口（字符设备）。

		3）ASoc Core
				ASoc是建立在标准ALSA Core基础上，为了更好支持嵌入式系统和应用于移动设备的音频Codec的一套软件体系。
